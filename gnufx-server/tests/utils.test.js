...
describe('Utils', () => {
    ...
    it('getToeData with correct userid ', () => {
        ...
        expect(utils.getToeData('1')).resolves.toEqual([{"feet": [{"toes": [{"images": [{"date": "2021-01-27", "fongiqueCoverage": "43%", "name": "0.PNG"}]}]}], "userId": "1"}]);
    });
    it('getToeData with wrong userid', () => {
        toeDataSchema.findOne = jest.fn().mockRejectedValueOnce([{userId: '1',feet : [{toes:[{images:[{date: "2021-01-27",name: "0.PNG",fongiqueCoverage: "43%"}]},]}]}]);
